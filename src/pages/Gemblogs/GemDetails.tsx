...
const GemDetails = () => {
    ...
    const navigateTo = useNavigate()
     ...
     const deleteGemblogMethod = () => {
        ...
        const { mutate:deleteGemblog, isLoading } = useMutation('delete-gemblog', deleteGemblogMethod, {
          onSuccess: (response:any) => {
            if(response?.status!==200){
              toast.error("Error Deletig!", { position:'bottom-right', duration: 2000 });
              return;
            }
              ...
              <div className="flex flex-col w-10/12 mb-20 mx-auto">
                <div className="flex justify-between items-center w-10/12 mx-auto mb-8">
                  <div className="flex flex-col space-y-4">
                    ...
                    <h1 className="font-medium font-serif text-4xl">{data?.data?.name}</h1>
                    ...
                    <div className="grid grid-cols-4 gap-8 py-6 border-t border-b border-gray-200">
                      <div className="flex flex-col items-center text-center space-y-2">
                        ...
                        <span className="text-sm">Alleviates Misfortune</span>
                      </div>
                      <div className="flex flex-col items-center text-center space-y-2">
                        <IoDiamondSharp className="w-5 h-5 text-red-500" />
                        <span className="text-sm">Supports Mental Health</span>
                      </div>
                      <div className="flex flex-col items-center text-center space-y-2">
                        <IoDiamondSharp className="w-5 h-5 text-red-500" />
                        <span className="text-sm">Increases Wisdom</span>
                      </div>
                      <div className="flex flex-col items-center text-center space-y-2">
                        <IoDiamondSharp className="w-5 h-5 text-red-500" />
                        <span className="text-sm">Success in Business</span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="danger">
                  ...
                  { isLoading ? <span className="text-red-600">Deleting....</span>: <ConfirmDelete onConfirm={deleteGemblog}/> }
                </div>
                {/* <GemstoneFilters /> */}
              </div>
            );
